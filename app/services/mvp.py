...
def _date(value, default: date) -> str:
    ...
    return (dt.date() if not pd.isna(dt) else default).isoformat()
...
def save_projects(records: Iterable[dict]) -> None:
    ...
    with connect() as conn:
        ...
        for r in records:
            ...
            (code,name,float(vals.get('row_km') or 0),float(vals.get('cct_km') or 0),float(vals.get('spus') or 0),float(vals.get('rs_hours') or 0),float(vals.get('gis_hours') or 0),float(vals.get('pls_hours') or 0),str(vals.get('start_date')),str(vals.get('end_date')),vals['loading_type'],str(vals.get('rs_start_date') or vals.get('start_date')),str(vals.get('gis_start_date') or vals.get('start_date')),str(vals.get('pls_start_date') or vals.get('start_date')),vals['status'],archived))
            keep.append(code)
...
def save_resources(records: Iterable[dict]) -> None:
    ...
    with connect() as conn:
        for r in records:
            ...
            (name,dept,float(r.get('weekly_hours') or 0),float(r.get('holiday_booked_hours') or 0),float(r.get('holiday_remaining_hours') or 0),status,r.get('status_reason'),r.get('status_start_date'),r.get('status_end_date')))
