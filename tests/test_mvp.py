...
class MvpWorkflowTests(unittest.TestCase):
    ...
    def test_resource_status_dates_for_editor_are_datetime_compatible(self):
        frame = pd.DataFrame([{'status_start_date': '2026-01-05', 'status_end_date': ''}])
        prepared = prepare_date_columns_for_editor(frame, RESOURCE_DATE_COLUMNS)
        self.assertTrue(pd.api.types.is_datetime64_any_dtype(prepared['status_start_date']))
    ...
    def test_saved_dates_use_iso_and_reload_as_editor_compatible(self):
        save_projects([{'project_code': 'D1', 'project_name': 'Dates', 'rs_hours': 1, 'gis_hours': 0, 'pls_hours': 0, 'start_date': pd.Timestamp('2026-05-01'), 'end_date': '1 May 2026', 'loading_type': 'even', 'rs_start_date': '', 'gis_start_date': None, 'pls_start_date': '01-May-26', 'status': 'active'}])
        saved = db.rows('SELECT start_date,end_date,rs_start_date,gis_start_date,pls_start_date FROM mvp_projects WHERE project_code="D1"')[0]
        ...
        reloaded = prepare_date_columns_for_editor(pd.DataFrame([saved]), PROJECT_DATE_COLUMNS)
